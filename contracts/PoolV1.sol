...
contract PoolV1 is Initializable, OwnableUpgradeable {
    uint public VERSION;
    ...
    function pumpPrice() public payable returns (uint) {
        ...
        if (interest == 0) {
            ...
            interest = balance / divider;
            divider = divider - 1;
        }

        if (interest > 0) {
            IBank(ADDRESS_BANK).updateBasePrice{value: interest}();
            lastPumpPriceTime = block.timestamp;
            emit PumpPrice(lastPumpPriceTime, interest);
        }
